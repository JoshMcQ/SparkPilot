...
1. **S3 gateway endpoint route table scope** — S3 gateway was being attached to all discovered route tables including public ones. Should be restricted to only private route tables by filtering against `var.private_subnet_ids`. (Two occurrences in main.tf)

2. **Missing SQS and STS interface endpoints** — Private control-plane tasks need SQS and STS endpoints for fully-private deployments without NAT. Suggested adding `aws_vpc_endpoint.sqs` and `aws_vpc_endpoint.sts`.

### Merge Decision

**Time from CodeRabbit review to merge: ~4.5 minutes.**

PR #113 was merged `~4.5 minutes` after CodeRabbit posted its review. Neither actionable finding was addressed before the merge.
...
PR #114 was opened to fix the production failure caused by PR #113. The root cause — strict `aws_route_table` data source failing for subnets with main-route-table associations — was a consequence of the route table lookup code introduced in #113.
...
Run `23774480703` was triggered after merging #114 → failed at `deploy-prod` because `AWS_PROD_DEPLOY_ROLE_ARN` was not set (prod environment not yet provisioned). This was a pre-existing issue unrelated to #113/#114 — addressed by PR #123 (#116).
...
## Lessons-Learned Checklist (mandatory for future PRs)
...
- [ ] **Check PR checklist** (once template is added per #117) before merging.
