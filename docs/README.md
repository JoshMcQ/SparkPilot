# SparkPilot Docs

This documentation set is organized as a product IA with five top-level paths.

## Start

- [Choose your mode](./setup/choose-mode.md)
- [BYOC-lite 30-minute quickstart](./setup/byoc-lite-30-minute-quickstart.md)
- [First live BYOC-lite run](./setup/first-live-run.md)
- [Live BYOC-lite from scratch](./runbooks/live-byoc-lite-from-scratch.md)
- [AWS auth quickstart](./setup/aws-auth-quickstart.md)

## Operate

- [Runbooks index](./runbooks.md)
- [Enterprise matrix real AWS runbook](./runbooks/enterprise-matrix-real-aws.md)
- [Live BYOC-lite real AWS proof (March 5, 2026)](./validation/live-byoc-lite-real-aws-proof-20260305.md)
- [Execution tracker](./execution-tracker.md)

## Integrate

- [Airflow provider integration](./integrations/airflow-provider.md)

## Govern

- [RBAC self-serve access validation](./validation/rbac-self-serve-access.md)
- [CUR chargeback progress](./validation/cur-chargeback-progress.md)
- [Issue closure checklist](./process/issue-closure-checklist.md)

## Troubleshoot

- [Runbooks troubleshooting section](./runbooks.md)
- [Troubleshooting matrix](./troubleshooting/matrix-remediation.md)
- [BYOC-lite real AWS matrix findings](./validation/byoc-lite-real-aws-test-matrix.md)
